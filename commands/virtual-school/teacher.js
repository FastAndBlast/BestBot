module.exports = {
    ...
    description: "Creates classes and adds them to teacher",
    ...
    run: async (client, message, args) => {
        ...
        for (i = 1; i < args.length; i++) {
            ...
            if (message.guild.roles.cache.find(roles => roles.name == className)) {
                ...
                const teacherRoleID = await message.guild.roles.cache.find(roles => roles.name === "Teacher").id;
        ...
        const teacherRole = await message.guild.roles.cache.find(roles => roles.name == "Teacher")

        user.roles.add(teacherRole);
        logMsg = logMsg + `${i}: Added ${user} to ${teacherRole} role`
